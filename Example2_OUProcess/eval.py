...
# Analytical: wider, more transparent
ax.plot(mu_range_dense, stat_var_analytical, color=colors[0], linewidth=3, alpha=0.5, label='Analytical')
...
for mu_test in mu_test_vals:
    ...
    x_final = model.generate_trajectory(x0=x0, mu=mu_test, n_steps=30, n_samples=n_samples_stat_var).flatten()
    learned_stat_var.append(np.var(x_final))

# Learned: markers
ax.plot(mu_test_vals, learned_stat_var, 'o', color=colors[1], markersize=5, label='Learned (30 steps)')
ax.set_xlabel(r'$\mu$', fontsize=16)
ax.set_ylabel('Variance', fontsize=16, rotation=0, ha='right')
ax.legend(fontsize=14)
...
# Analytical: wider, more transparent
ax.plot(mu_range_dense, cond_var_analytical, color=colors[0], linewidth=3, alpha=0.5, label='Analytical')
...
# Learned: markers
ax.plot(mu_test_vals, learned_cond_var, 'o', color=colors[1], markersize=5, label='Learned')
ax.set_xlabel(r'$\mu$', fontsize=16)
# Remove y-axis for right panel (shared y-axis with left)
ax.set_yticklabels([])
ax.tick_params(axis='y', length=0)
ax.spines['left'].set_visible(False)
